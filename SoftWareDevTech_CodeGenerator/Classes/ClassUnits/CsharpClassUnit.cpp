...
void CsharpClassUnit::add(const std::shared_ptr<IUnit> &unit, IUnit::Flags flags) { //  Функция добавления Unit в список полей
...
std::string CsharpClassUnit::compile(unsigned int level) const {
    ...
    for(size_t i = 0; i < ACCESS_MODIFIERS.size(); i++) { //  Движение по доступным модификаторам доступа
        ...
        for(const auto& field : m_fields[i]) {
            ...
            result += ACCESS_MODIFIERS[i]; //  Вывод модификатора доступа непосредственно перед методом (в С#)
            result += " ";
            result += field->compile(level); //  Компиляция всех методов с вышеуказанным модификатором (в C#)
            result += "\n";
        }
    }
